...
#### 기능 개선/변경
* 인증서 API 추가
	* 인증서 발급/조회/삭제 API가 추가되었습니다. 자세한 내용은 [API v2.0 가이드 > 인증서 API](./api-guide-v2.0/---)를 참고하시기 바랍니다.
* 통계 API 추가
	* 네트워크 전송량, HTTP 상태 코드별 통계 및 다운로드가 가장 많은 콘텐츠의 순위 통계를 조회할 수 있는 API가 추가되었습니다. 자세한 내용은 [API v2.0 가이드 > 통계 API](./api-guide-v2.0/---)를 참고하시기 바랍니다.
