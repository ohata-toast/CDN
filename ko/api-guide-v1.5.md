...
APIを使用するには、アプリキー(Appkey)とセキュリティキー(SecretKey)が必要です。 
...
| distributions[0].domainAlias           | String  | 任意   |        | 最大255文字            | ドメインエイリアス(個人または会社が所有しているドメイン使用、複数入力時は\nトークンで区切って入力してください。) |
...
| domainAlias           | String  | 任意   |        | 最大255文字                                             | ドメインエイリアス(個人または会社が所有しているドメイン使用、複数入力時は\nトークンで区切って入力してください。) |
...
サービスの一部の設定を変更する場合、部分修正APIを利用できます。
...
| forwardHostHeader     | String  | 任意  |        | ORIGIN_HOSTNAME/REQUEST_HOST_HEADER   | CDNサーバーがオリジンサーバーにコンテンツをリクエストする時に伝達するホストヘッダ設定("ORIGIN_HOSTNAME"：オリジンサーバーのホストネームで設定、"REQUEST_HOST_HEADER"：クライアントリクエストのホストヘッダで設定)|
...
| domainAlias           | String  | 任意   |        | 最大255文字                                             | ドメインエイリアス(個人または会社が所有しているドメイン使用、複数入力時は\nトークンで区切って入力してください。) |
| maxAge                | Integer | 任意  | 0      | 0～2,147,483,647                                              | キャッシュ満了時間(秒)、デフォルト値0は604,800秒です。           |
| referrers             | String  | 任意   |        | '\n'トークンを含めて最大1024文字                            | リファラー(複数入力時は\nトークンで区切って入力してください。 )  |
| origins               | List    | 任意   |        |                                                              | オリジンサーバー                                             |
| origins[0].origin     | String  | 任意   |        | 最大255文字                                             | オリジンサーバー(domainまたはIP)                                     |
| origins[0].port       | Integer | 任意  |        |[コンソール使用ガイド] > [[オリジンサーバー](./console-guide/#_2)の[表2]使用可能なオリジンサーバーポート番号]参照| オリジンサーバーHTTPプロトコルポート<br>(origins[0].port設定時、origins[0].httpPortとorigins[0].httpsPortは入力しません。)|
| origins[0].httpPort   | Integer  | 任意  |        |[コンソール使用ガイド] > [[オリジンサーバー](./console-guide/#_2)の[表2]使用可能なオリジンサーバーポート番号]参照 | オリジンサーバーHTTPプロトコルポート<br>(origins[0].port未設定時、origins[0].httpPortとorigins[0].httpsPortのいずれか1つは必ず入力する必要があります。)  |
| origins[0].httpsPort  | Integer  | 任意  |        |[コンソール使用ガイド] > [[オリジンサーバー](./console-guide/#_2)の[表2]使用可能なオリジンサーバーポート番号]参照 | オリジンサーバーHTTPSプロトコルポート<br>(origins[0].port未設定時、origins[0].httpPortとorigins[0].httpsPortのいずれか1つは必ず入力する必要があります。) |
| origins[0].originPath | String  | 任意   |        | 最大8192文字                                            | オリジンサーバーの下層パス                                   |
...
| purgeList | String | 任意   |        |                       | 再配布対象項目リスト(複数入力する時は\nトークンで区切って入力してください。purgeTypeがALLの場合は入力しなくても構いません。) |
...
| header.resultCode    | Integer | 結果コード  |
| header.resultMessage | String  | 結果メッセージ |
| purgeSeq             | Integer | 再配布リクエスト番号 |

- CDNサービスを新規で作成した後、約1時間はキャッシュ再配布リクエストは失敗する場合があります。その後も失敗が続く場合はサポートへお問い合わせください。
- パージAPIは使用量制限ポリシーがあります。詳細な内容は[コンソール使用ガイド] > [[CDNキャッシュ再配布](https://docs.toast.com/ko/Contents%20Delivery/CDN/ko/console-guide/#cdn_3)のキャッシュ再配布使用量制限]を確認してください。
...
| purges[0].seq           | Integer | 再配布リクエスト番号                  |
| purges[0].progress      | Integer | 再配布進行率                     |
| purges[0].purgeTime     | Long    | 再配布リクエスト時間                  |
| purges[0].lastCheckTime | Long    | 再配布実行の最終チェック時間           |
| purges[0].type          | String  | 再配布タイプ("ITEM"、"WILDCARD"、"ALL") |
| purges[0].path          | String  | 再配布リクエスト項目                  |
