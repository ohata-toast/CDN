...
### 메서드
CDN에서 기본적으로 허용하고 있는 메서드는 GET, HEAD, OPTIONS 으로, 이외의 메서드를 요청하면 거절됩니다.
해당 메서드 이외의 메서드를 허용하고자 할 경우 원하는 메서드를 선택하여 설정합니다.
![CDN서비스생성-루트경로](https://static.toastoven.net/prod_cdn/v2/console-cdn-create-root-path_1.png)

- **메서드 허용 설정**
  허용 설정한 메서드 요청은 캐시되지 않고 원본 서버로 전달됩니다.
...
- **캐시 설정**
  CDN 서버가 원본 파일을 캐싱할 때 어떤 캐시 설정을 이용할 지 설정 할 수 있습니다.
    ...
    - **Bypass Cache**: 해당 설정 이전에 생성된 캐시는 유지한 채, 이후 컨텐츠 요청을 캐시하지 않습니다.
